> **TRANSPORT COPY** — snapshot taken 2026-07-29 for the ATOP return capsule.
> The canonical, editable original is ATOP `C:\Users\astro\fita\FITA_FORMAT_STANDARD.md`.
> Do not edit this copy; if it and the original differ, the original governs.
>
> **BTOP landing note (2026-08-01):** received by email and written here because BTOP had
> no copy of the normative standard at all. This is the reference against which
> `FITA_FOUNDRY_DESIGN.md` v0.2 was conformed. Still a transport copy — the ATOP original governs.

# FITA — Flexible Image Transfer Alpha
## Format Standard Document — v1.1 DRAFT

| Field | Value |
|---|---|
| **Document status** | DRAFT — normative, pending Ignacio's rulings on the items in §12 |
...
No such checker exists in v1.0; the absence of one is why the defects in §11 went undetected through
110 passing tests.
...
`[NEW]` This standard deliberately does **not** specify the parallax scaling (the map from `ZDP` to
pixel offset). The MWVO depth-stimulus discipline requires stereo separation to be *measured* and
tied to a real→model metric chain, which is a property of a rendering job, not of a file. A future
`FITA_ZSC` keyword could record the scaling actually used — see D-6.
...
Conformant wording until the gap is closed: *"an ObsCore-derived provenance subset"*. Whether to
...
## 12. Open decisions — for Ignacio only

These are the calls this document deliberately does **not** make.
...
| 1.1 | 2026-07-29 | **this document, DRAFT** | Alpha `BZERO` correction; `BUNIT` correction; `FITA_VIS`; `SPLIT16` deprecated; registry authority fixed; ObsCore overclaim withdrawn; conformance levels; FITO boundary |
